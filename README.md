...
⑤ 契約頁 OCR（employer_extract + address_db）→ 雇主名稱/地址/電話 + 官方譯名/郵遞區號（H~O）
    ▼
⑤b 仲介反查救援：契約次頁『台灣仲介』電話（優先）→ 名稱/地址相似度（後備）→ 反查名冊回推許可證（B + E~G）
    ▼
...
1. 找契約頁的**次一頁**，整頁送 Vision OCR（**每件僅 1 次** Vision；按張計費，
   整頁與裁切同額度），一次取回台仲區塊的**電話、名稱、地址**三項。
...
2. **只取「台灣仲介」，排除印尼仲介（P3MI）**：台印格式重疊、無法靠格式或位置分，
   ...
   仲介欄位標籤或印尼電話標籤（Telp/Telepon）為止，只抽此區塊。台仲**左右不定**
   （32345 在右、32324 在左）故不靠幾何裁半；「真標籤」以「短行＋非句號結尾」判定，
   濾掉 prose 誤配。找不到真台仲標籤 → 回空**不猜**（不誤抓印尼資料）。
3. **反查名冊，電話優先**：
   - **電話**（`agencies_by_phone`，精確反查；支援 `+886` 國際格式與雙連字號正規化）：
     唯一命中 → 回填 B；同電話多家（名冊約 2.6%）→ 不填 B、列出各家供人工擇一。
   - **（後備）名稱／地址相似度反查**：電話查無或多家時啟用。從同一份次頁 OCR 另取
     台仲**名稱、地址**，分別與名冊「機構名稱／地址」正規化後比**相似度**（difflib），
     取**相似度（CONF）高者**回填 B——名稱門檻 `NAME_MATCH_MIN`(0.78)、
     地址 `ADDR_MATCH_MIN`(0.72)，達門檻才採用（`best_by_name` / `best_by_address`）。
     名稱會先以 `_clean_agency_name` 掐掉英文/符號雜訊（`LTD(裕倉…` → `裕倉…`）。
   - 三者皆無 → 維持人工。
...
**已知限制**：電話若被紅章蓋到讀不出（如 32345），電話這條救不回；但**名稱或地址
仍讀得到時可由後備管道救回**（實測 32397 電話空，靠地址相似度 1.00 反查命中 2449）。
仍保證不誤抓印尼機構——寧可回空，不填錯。

> 追查救援結果：Sheets **D 欄 reason** 會標「[電話反查救回] 次頁台仲電話 X → 許可證 Y／機構名」、
> 「[名稱反查救回·電話無唯一命中] …（名冊相似度 Z）」、「[地址反查救回…（名冊相似度 Z）」
> 或「[電話反查·需人工擇一] …命中 N 家：…」。
