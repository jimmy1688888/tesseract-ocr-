...
⑤ 契約頁 OCR（employer_extract + address_db）→ 雇主名稱/地址/電話 + 官方譯名/郵遞區號（H~O）
    ▼
⑤b 全無命中救援：契約次頁『台灣仲介』電話 → 反查名冊 → 回推許可證（B + E~G）
    ▼
...
| D | **全部圖片無命中** | **人工審查**（不對許可證 ROI 送 Vision），但先走**電話反查救援**（見下節） | 沒有候選值可交叉驗證，硬讀許可證只會是單引擎；改用「仲介電話反查名冊」這條獨立管道回推 |

**small docx**（圖片數 ≤ 3，版面固定）較簡單：mol 多數票且平均信心 >
`CONF_MOL_VOTE_MIN`(50) → 直接 key-in；其餘有值 → 送 Vision；全無命中 → 人工審查（同樣先走電話反查救援）。
...
### 全無命中救援：仲介電話反查名冊回推許可證

**動機**：許可證數字整份掃不到（章糊、影印歪斜）時，B 欄原本只能空著等人工。
但契約**次一頁**通常印有「台灣仲介公司」區塊，含**仲介電話**——而名冊
（`agency_roster.json`）本來就有每家仲介的電話。於是多開一條**與許可證數字無關**
的獨立管道：**讀仲介電話 → 反查名冊 → 回推許可證**。

流程（`pipeline.rescue_manual_review_via_agency_phone` + `employer_extract.agency_phones_from_next_page`）：

1. 對每個 manual_review（全無命中）件，找契約頁的**次一頁**，整頁送 Vision OCR
   （**每件僅 1 次** Vision；Vision 按張計費，整頁與裁切同額度）。
2. **只取「台灣仲介」的電話，排除印尼仲介**：次頁通常台仲與印尼仲介（P3MI）
   兩格並列，且**台印電話都以 0 開頭、格式重疊，無法靠格式區分**。改以**語意錨定**：
   - 以「台灣仲介公司(Agency Taiwan)」**欄位標籤**為起點，到其後的仲介欄位標籤或
     印尼電話標籤（Telp/Telepon）為止，只抽這個區塊內的電話。
   - **台仲區塊左右不定**（實測 32345 在右、32324 在左），故**不靠幾何裁半**、
     改整頁 OCR 靠標籤定位。找不到真台仲標籤 → 回空**不猜**（寧可不救，不誤抓印尼電話）。
   - 「真欄位標籤」以「短行＋非句號結尾」判定，濾掉契約樣板 prose 裡
     「…另一份由台灣仲介公司存查。」「agency Taiwan…」這類**誤配**。
3. **電話正規化**支援 `+886` 國際格式（`+886 2 2727 6999` = 國內 `02-27276999`，
   去國碼、補回前導 0）與號段間雙連字號（`02-2727-6999`）。
4. **反查名冊**（`permit_lookup.agencies_by_phone`）：
   - **唯一命中** → 回填 B 欄許可證，E~G 由該許可證查名冊得**官方機構名**
     （不受次頁仲介名 OCR 糊化影響）；狀態仍為 `manual_review`、reason 註明
     「電話反查救回…仍請人工核對」。
   - **同電話多家仲介**（名冊約 2.6%）→ **不自動填 B**，把每家（許可證／名稱）
     列進 reason，維持人工擇一。
   - 查無 → 維持原全無命中狀態。

**設計原則不變**：電話反查是**另一條獨立證據管道**，不違反「單一 OCR 引擎不得
直接決定寫入」——它救回的值仍標 `manual_review` 要人工過目，且 E~G 用的是名冊
官方資料而非 OCR 名稱。

**已知限制**：仲介電話若也被紅章蓋到 OCR 讀不出（如 32345），這條一樣救不回，
但**保證不會反查到錯的（印尼）機構**——回空而非填錯。

> 追查救援結果：Sheets **D 欄 reason** 會標「[電話反查救回] 次頁台仲電話 X →
> 許可證 Y／機構名」或「[電話反查·需人工擇一] …命中 N 家：…」。
